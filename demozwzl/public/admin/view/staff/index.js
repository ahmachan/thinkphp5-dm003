...
$(".menu_buttons .menu_3").on("click",function(){
	  ...
	  }else if(node.text().indexOf("借卡")>-1){
	  	   var $ids=[];
	  	  $("#data-table input[name='ids']:checked").each(function(){
	  	  	 $ids.push($(this).val());
	  	  });
	  ...
	  }else if(node.text().indexOf("还卡")>-1){
	  	   var $ids=[];
	  	  $("#data-table input[name='ids']:checked").each(function(){
	  	  	 $ids.push($(this).val());
	  	  });
...
    getQueryCondition : function(data) {
	...
	        }
	        //组装查询参数
	     	    param.a_name = $("#myform #name").val();
						param.a_mobile = $("#myform #mobile").val();
						param.a_card_no= $("#myform #card_no").val();
        ...
        columns: [
            ...
            },
            {
...

             {
                	...
                	if(data==null||data.length<5){return "<span>否</span>";}else{return "<sapn class='label label-danger'>是</span>";}
                },
