...
$(".menu_buttons .menu_3").on("click",function(){
	  ...
	  if(node.text().indexOf("设置免指纹")>-1){
	  	   $("#myModalBody").load(node.attr("data-url"),function(){
	  	   	      $("#myModal").modal("show");
	  	   });
	  }else if(node.text().indexOf("验卡")>-1){
	  	    $("#myModalBody").load(node.attr("data-url"),function(){
	  	   	$("#myModal").modal("show");
	  	   });
	  }else if(node.text().indexOf("绑卡")>-1){
	  	   var $ids=[];
	  ...
	  }else if(node.text().indexOf("查询卡")>-1){
	  	   var $ids=[];
	  	  $("#data-table input[name='ids']:checked").each(function(){
	  	  	 $ids.push($(this).val());
	  	  });
        ...
        columns: [
            ...
            {	
               ...
               render : function(data,type, row, meta){
                	if(data==null||data==''||data==0){return "<span class='label label-danger'>未绑卡</span>";}else{return data;}
                },
            },
            {
                data: "name",
            ...
            {
                data: "in_varifycation",
                render : function(data,type, row, meta){
                	if(data!=1){return "<span class='label label-info'>时限卡</span>";}else{return "<span class='label label-warning'>次卡</span>";}
                },
            },
            {	
                data: "start_date",
