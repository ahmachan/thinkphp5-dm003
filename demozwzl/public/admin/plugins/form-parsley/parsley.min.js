	...
	var h = function(a) {
		this.messages = {
			...
			type: {
				...
				number: "请输入有效的数值.",
				digits: "请输入有效的数字",
				dateIso: "日期类型格式必须是 (YYYY-MM-DD).如2017-12-10",
				alphanum: "请输入阿拉伯数字",
				phone: "请输入电话号码."
			},
			notnull: "值不能为空",
			notblank: "值不能为空",
			required: "必填选项.",
			regexp: "数据格式不正确 请重新输入",
			min: "您输入的数据不能小于 %s.",
			max: "您输入的数据不能大于 %s.",
			range: "数据的范围必须在 %s 和 %s 之间",
			minlength: "您输入的数据不能小于 %s 位.",
			maxlength: "您输入的数据不能大于 %s 位.",
			rangelength: "数据长度必须在  %s 位和 %s 位之间.",
			mincheck: "您至少选择 %s 个选项.",
			maxcheck: "您最多选择  %s 个选项.",
			rangecheck: "选项个数必须 在 %s 到 %s 之间.",
			equalto: "二次输入的值必须一致."
		};
		this.init(a)
	};
	...
	j.prototype = {
		...
		applyValidators: function() {
			...
			for(b in this.constraints) {
				...
				!1 === c ? (a = !1, this.constraints[b].valid = a, this.options.listeners.onFieldError(this.element, this.constraints, this)) : !0 === c && (this.constraints[b].valid = !0, a = !1 !== a, this.options.listeners.onFieldSuccess(this.element, this.constraints,
					this))
			}
			return a
		},
